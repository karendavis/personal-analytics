...
class ApplicationItem:
    ...
    def dict(self, index):
        return {
            f"application_name_{index}": self.application_name,
            f"application_hour_{index}": self.application_hour,
...
def filter_empty_text_items(text_items: list) -> list:
    ...
    text_items = [item.replace("Amin", "4min") for item in text_items]
    ...
    # If there are more than two instances of 'safari' in the image, the first is in the top left
    # of the screen and not part of the applications with time section. If there are more than 1
...
def create_screen_time_item_from_text(text_items: list) -> ScreenTimeItem:
    screen_time_date_indexes = [
        i
        for i, item in enumerate(text_items)
        if "Today," in item or "Yesterday," in item
    ]
    if len(screen_time_date_indexes) == 0:
        ...
        screen_time_date_idx = [
            i
            for i, item in enumerate(text_items)
            if re.search(day_of_week_pattern, item)
        ][0]
    else:
    ...
    hours, minutes = get_hours_and_minutes(
        text_items, screen_time_date_idx + 2, seconds_round_up=3
    )
    return ScreenTimeItem(
        day=text_items[screen_time_date_idx + 1],
...
def convert_string_to_digit(string, digit_type):
    ...
    try:
        ...
        logging.warning(
            f"Could not create int from  {digit_type} time string, using 0 instead"
        )
        digit = 0
    return digit
